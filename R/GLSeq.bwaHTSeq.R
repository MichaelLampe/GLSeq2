...
for (zz in 1:nStreams) {
  ...
  for (i in rangelist[[zz]]) {
    ...
    #
    # current command: 
    if (paired.end) comm.i <- paste(index, "&&", aln.left, "&&", aln.right, "&&", sam.create, "&&", cleansam.comm, "&&", finalsam.comm, "&&", bam.create, "&&", bam.index, "&&", countable.comm, "&&", count.comm, "&&", spaceCleanup)
    if (!(paired.end)) comm.i <- paste(index, "&&",aln.left, "&&", sam.create, "&&", cleansam.comm, "&&", finalsam.comm, "&&", bam.create, "&&", bam.index, "&&", countable.comm, "&&", count.comm, "&&", spaceCleanup)
