...
public class GlowDataRequest extends GlowRequest {
  ...
  private ArrayList<CheckBoxTreeItem<String>> addFilesToUi(List<String> files) {
    try {
      TreeView<String> fileView = (TreeView<String>) UpdateUI.getInstance().findById(
          "selectedDataFiles");
      for (String file : files) {
        ...
        // If we want to check the selected property we'll need to make it into
        ...
        b.selectedProperty().addListener(new ChangeListener<Boolean>() {
          ...
          public void changed(ObservableValue<? extends Boolean> arg0, Boolean oldValue,
            ...
            /*
             * This gets activated when a change happens Changes all the
             * TreeItems to be equal to their path branch
             */
  ...
  private void addBoxListeners(ArrayList<CheckBoxTreeItem<String>> files) {
    for (CheckBoxTreeItem<String> file : files) {
      file.selectedProperty().addListener(new ChangeListener<Boolean>() {
        ...
        public void changed(ObservableValue<? extends Boolean> arg0, Boolean oldValue,
            Boolean newValue) {
          // Add and remove the string depending on if it is checked or not
          if (newValue) {
