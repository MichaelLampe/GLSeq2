	...
	protected void updateAttributes() {
		...
		for (Attribute attribute : array) {
			...
			if (currentData instanceof Slider) {
				attribute.setValue(String.valueOf(Integer.valueOf((int) ((Slider) currentData).getValue())));
				// Combo boxes we just want the selected item.
			} else if (currentData instanceof ComboBox<?>) {
				attribute.setValue(
						String.valueOf(((ComboBox<?>) currentData).getSelectionModel().getSelectedItem().toString()));
				...
				// settings.
				// Run settings are not in the attribute file, so let's just
				// format
