...
public class GlowRequest extends Task<Object> {
	...
	boolean requestCookie() {
		...
		ArrayList<String> command = new ArrayList<String>() {
			{
				...
				add(cookie);
				add("--data");
				add("user[login]=" + username + "&user[password]=" + password
						+ "&commit=Sign+in");
		...
		try {
			 ...
			 * future proofs it a bit. If the password is wrong it returns the
			 * whole html page so even 200 should be pretty safe. The greater
			 * than protects us against incorrectly directed requests that don't
			 * do anything Nor get anything back making the program think they
			 * are correct
			 */
